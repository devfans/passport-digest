...
var passport = require('passport-strategy')
  ...
  , sha512256 = require('js-sha512').sha512_256
  , util = require('util');
 ...
 *   - `algorithm`  algorithm used to produce the digest (MD5 | SHA-256 | SHA-512-256 or with -sess)
...
DigestStrategy.prototype.authenticate = function (req) {
  ...
  // value.
  this._secret(creds.username, (err, user, password) => {
    if (err) { return self.error(err); }
    if (!user) { return self.fail(self._challenge()); }

    var ha1;
    var hashFunc;
    if (!creds.algorithm || creds.algorithm.startsWith("MD5")) {
      hashFunc = md5;
    } else if (creds.algorithm.startsWith("SHA-256")) {
      hashFunc = sha256;
    } else if (creds.algorithm.startsWith("SHA-512-256")) {
      hashFunc = sha512256
    } else {
      return this.fail(this._challenge());
    }

    if (creds.algorithm && this._algorithm && creds.algorithm != this._algorithm) {
      return this.fail(this._challenge());
    }

    if (!creds.algorithm || !creds.algorithm.endsWith("-sess")) {
      if (typeof password === 'object' && password.ha1) {
        ha1 = password.ha1;
      } else {
        ha1 = hashFunc(creds.username + ":" + creds.realm + ":" + password);
      }
    } else {
      ...
      ha1 = hashFunc(hashFunc(creds.username + ":" + creds.realm + ":" + password) + ":" + creds.nonce + ":" + creds.cnonce);
    }

    var ha2;
    if (!creds.qop || creds.qop === 'auth') {
      ha2 = hashFunc(req.method + ":" + creds.uri);
    ...
    var digest;
    if (!creds.qop) {
      digest = hashFunc(ha1 + ":" + creds.nonce + ":" + ha2);
    } else if (creds.qop === 'auth' || creds.qop === 'auth-int') {
      digest = hashFunc(ha1 + ":" + creds.nonce + ":" + creds.nc + ":" + creds.cnonce + ":" + creds.qop + ":" + ha2);
    } else {
      return self.fail(400);
    }
